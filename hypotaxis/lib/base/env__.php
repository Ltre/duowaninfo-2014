...
//界定是否处于开发模式
if (false !== stripos($_SERVER['HTTP_HOST'], 'doinject.duapp.com')) {
	define('DEV_MODEL', false);//在服务器上
} else {
	define('DEV_MODEL', true);
}
 ...
 * URL SHELL 的含义：见ActionUtil::numOfShellArgs()  @  /lib/action__.php
 ...
 * 如果想要通过URL[http://server.com/ItemName/?xxx=shell|*]的形式调用其中的方法，则需要配置ActionUtil::numOfShellArgs() @ lib/base/action__.php，详见注释。
...
/*
 * 用户扩展库文件默认保存位置是'lib/ext'。
 * 格式：路径要以项目根目录为起点，不能以“/”开头，必须以“/”结尾。
 * 如果要配置多个保存位置，就需要用“|”隔开。如：'lib/ext/|lib/abc/|abc/def/'。
 */
define('USER_LIB_DIRS', 'lib/ext/');

/*
 * php的const、define、URL指令等等的文件位置，这些文件名格式为：*.php。
 * 默认值为：'config/'。
 * 格式：路径要以项目根目录为起点，不能以“/”开头，必须以“/”结尾
 * 如果要配置多个保存位置，就需要用“|”隔开。如：'config/|config/myconfig/'。
 */
define('USER_CONFIG_DIRS', 'config/');

/*
 * XxxxAction默认模板目录，用于与输出Action方法同名的模板文件。
 * 默认保存于/tpl/default/。
 * 模板文件保存形式：先在tpl/default/新建名为Action名称的文件夹，再在这个文件夹中新建与Action方法同名的模板文件，后缀名限定为.php。
 * 输出方式：必须由Action的tpl()方法输出。链接将无法直接访问这类文件。
 * 参数支持：由XxxxAction的基类ActionUtil::getTplArgs() 提供带索引数组形式的参数，以供模板文件内脚本使用。
 */
define('ACTION_TEMPLATE_DIR', 'tpl/default/');
 ...
 * 如：http://server.com/ItemName/?xxx=a-b-c 将默认访问到tpl/a/b/c.php。
 * 因此模板文件名的命名规范是：1、不含“-”；2、必须以.php为后缀名。
 */
define('OTHER_TEMPLATE_DIR', 'tpl/other/');
 ...
 * 以下是示例常量
 */
define('JQUERY_LIB_PATH', 'res/js/jquery-1.8.3.min.js');	//jQuery.js路径
